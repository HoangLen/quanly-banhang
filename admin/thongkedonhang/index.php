...
switch($action){
    case "macdinh": 
		...
		$donhang= $dh->laydonhangtheokhoangthoigian($date1,$date2);
		include("main.php");
       ...
       if(isset($_GET["date"]))
		{
			...
			$date = $ngay.' 00:00:00';
			$now = $ngay.' 23:59:59';							
			$date_dh = $dh->laydonhangdagiaotheokhoangthoigian($date,$now);			        	
            include("group.php");
        }
        else{
            include("main.php");
	...
	case "xemchitiet": 
        if(isset($_REQUEST["madh"])){
            $madh = $_REQUEST["madh"];
			$d_hang = $dh->laydonhangtheoid($madh);
            $tendh =  $d_hang["id"];  
            $dhang = $ct->laydonhangct($madh);
